...
- 2026-07-22: The completion audit also corrected stale documentation that still described planner/API candidates as unsupported. Current code classifies runtime surfaces, deploys `Dockerfile.candidate` in the isolated Railway project, creates a public domain only for API candidates, verifies the exact candidate health/version, and routes eligible creation/resume requests through stable control-side cohort selection. Frontend candidates remain truthfully unsupported until an isolated preview router exists; worker-only evidence cannot attest an API/frontend candidate.
