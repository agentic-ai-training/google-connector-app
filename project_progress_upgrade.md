...
- [~] Run GitHub Actions, Railway, Neon, Vercel, Google OAuth, Grafana Cloud, Alloy, and LangSmith production smoke tests where credentials/account state allow. Railway API/worker, Neon migration 008, Vercel, OAuth redirect/PKCE/scopes, metrics, logs, traces, alerts, dashboards, authorization, and LangSmith pass; only the newest GitHub workflow completion and time/data-dependent pilot conclusions remain pending.
