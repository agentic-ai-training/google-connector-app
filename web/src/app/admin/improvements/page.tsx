...
export default function Improvements(){
  ...
  return <main className="mx-auto min-h-screen max-w-5xl bg-zinc-50 p-6 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
    ...
    <section className="mb-8 rounded-xl border bg-white p-5 dark:bg-zinc-900"><h2 className="text-xl font-semibold">Candidate builder</h2><p className="text-sm text-zinc-500">Groq generates frozen files with bounded repository tools. Generated code is never run while the Groq key is present; trusted no-secret CI validates it.</p><div className="mt-3 overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr><th className="p-2">Proposal</th><th className="p-2">Mode/model chain</th><th className="p-2">Status and phase</th><th className="p-2">Budget and progress</th><th className="p-2">Resume</th><th className="p-2">Failure</th></tr></thead><tbody>{candidateBuilds.map(build=><tr key={build.id} className="border-t align-top"><td className="p-2">{build.proposal_key}</td><td className="p-2">{build.mode}<div>{(build.models_used?.length?build.models_used:[build.model_name]).join(" → ")}</div><div className="text-zinc-500">Configured primary: {build.model_name}<br/>{build.model_policy_version}<br/>{build.tool_policy_version}</div></td><td className="p-2"><div>{build.status.replaceAll("_"," ")}</div><div className="text-zinc-500">{build.generation_phase?.replaceAll("_"," ")??"not checkpointed"}{build.active_role?` · ${build.active_role.replaceAll("_"," ")}`:""}{build.next_round!==undefined&&build.next_round!==null?` · next round ${build.next_round}`:""}</div><div className="text-zinc-500">Dispatch: {build.retry_dispatch_state?.replaceAll("_"," ")??"not dispatched"}</div></td><td className="p-2"><div>Base {build.base_token_budget.toLocaleString()} · Effective {build.effective_token_budget.toLocaleString()}</div><div>Usage: cumulative {build.cumulative_tokens.toLocaleString()} · current role {build.active_role_tokens.toLocaleString()}</div><div className="text-zinc-500">{build.staged_file_count} staged/frozen · {build.tool_calls} tools · {build.read_bytes.toLocaleString()} read bytes · {build.json_tool_protocol?"JSON protocol":"native protocol"}</div><div className="text-zinc-500">Gate: {build.progress_gate?.replaceAll("_"," ")??"not reached"}{build.last_tool_name?` · last ${build.last_tool_name}`:""}</div></td><td className="p-2"><div>{build.retryable?"Provider retry eligible":"Provider retry unavailable"}: {build.retry_reason.replaceAll("_"," ")}</div><div className="text-zinc-500">{build.resume_point?.phase?`Resume ${build.resume_point.phase.replaceAll("_"," ")}`:"No safe resume point"}{build.resume_point?.active_role?` · ${build.resume_point.active_role.replaceAll("_"," ")}`:""}{build.resume_point?.next_round!==undefined?` round ${build.resume_point.next_round}`:""}{build.resume_point?.staged_file_count!==undefined?` · ${build.resume_point.staged_file_count} frozen files`:""}</div>{build.retryable&&<div className="text-zinc-500">{build.next_retry_at?`Eligible ${new Date(build.next_retry_at).toLocaleString()}`:"waiting for scheduler"}</div>}</td><td className="p-2"><div>{build.error_message??"—"}</div>{build.retry_error_type&&<div className="text-zinc-500">{build.retry_stage??"runner"} · {build.retry_error_type}</div>}{build.contract_errors.length>0&&<div className="text-zinc-500">Contract: {build.contract_errors.join(", ")}</div>}{build.new_policy_retry_available&&<button onClick={()=>safely(()=>retryWithCurrentPolicy(build))} className="mt-2 rounded border px-2 py-1">Create current-policy attempt</button>}</td></tr>)}</tbody></table></div></section>
