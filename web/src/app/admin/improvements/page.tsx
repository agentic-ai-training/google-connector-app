...
type CandidateBuild={id:string;proposal_key:string;title:string;mode:"single"|"multi_role";status:string;lifecycle_label:string;superseded_by_policy:boolean;model_name:string;models_used:string[];model_policy_version:string;tool_policy_version:string;tokens_used:number;token_budget:number;base_token_budget:number;effective_token_budget:number;cumulative_tokens:number;active_role_tokens:number;remaining_effective_tokens:number;error_message?:string;created_at:string;updated_at:string;file_count:number;staged_file_count:number;tool_calls:number;read_bytes:number;json_tool_protocol:boolean;last_tool_name?:string;contract_errors:string[];progress_gate?:string;retryable:boolean;new_policy_retry_available:boolean;retry_reason:string;build_admission:"eligible"|"evidence_required";admission_reason_codes:string[];resume_point?:{phase?:string;active_role?:string;next_round?:number;staged_file_count?:number};retry_count:number;retry_stage?:string;retry_error_type?:string;retry_after_seconds?:number;next_retry_at?:string;retry_dispatch_state?:string;generation_phase?:string;active_role?:string;next_round?:number;coding_run_id?:string;coding_run_status?:string;coding_current_phase?:string;coding_approval_action_hash?:string;coding_approval_expires_at?:string;coding_approval_preview?:{changes?:{path?:string;before_sha256?:string;after_sha256?:string}[];validation_profiles?:string[]};coding_pull_request_url?:string;coding_ci_check_url?:string};
...
export default function Improvements(){
  ...
  const load=async()=>{
    ...
    setProposals(newestFirst(proposalData.proposals));
    await requireAuthorized(notificationResponse,"Unable to load the notification ledger");
    setNotifications(newestFirst((await notificationResponse.json()).notifications));
    await requireAuthorized(incidentResponse,"Unable to load the failure review inbox");
    const incidentData=await incidentResponse.json();
    setIncidents(newestFirst(incidentData.incidents));
    setFailureNotifications(newestFirst(incidentData.notifications));
    await requireAuthorized(buildResponse,"Unable to load candidate builds");
    setCandidateBuilds(newestFirst((await buildResponse.json()).builds));
  ...
  const decideIncident=async(incident:FailureIncident,decision:"choose_A"|"choose_B"|"acknowledged"|"ignored")=>{
    ...
    const data=await response.json();
    if(!response.ok)throw new Error(data.detail??"Failure decision failed");
    const buildStatus=data.proposal?.candidate_build_status;
    setMessage(decision.startsWith("choose_")?(buildStatus==="queued"?"Strategy selected. A source-grounded Groq-only candidate build was queued. It cannot execute its code; trusted CI and human canary approval remain required.":buildStatus==="evidence_required"?"Strategy recorded as diagnosis only. More specific failure evidence is required before code generation; no model build was started.":"Strategy recorded; candidate automation is currently disabled."):"Failure review recorded.");
    await load();
  };
  ...
  const retryWithCurrentPolicy=async(build:CandidateBuild)=>{
    if(!window.confirm("Create a fresh candidate attempt from sanitized evidence using the current builder policy? The failed build remains immutable."))return;
    const response=await fetch(`${API}/admin/candidate-builds/${build.id}/new-policy-attempt`,{
      method:"POST",headers:{...headers(),"Content-Type":"application/json"},
      body:JSON.stringify({confirmation:"RETRY WITH CURRENT BUILDER POLICY"}),
    });
    const data=await response.json();
    if(!response.ok)throw new Error(data.detail??"Unable to create a new policy attempt");
    setMessage(`New governed candidate attempt ${data.build_id} was queued. No code was published or deployed.`);
    await load();
  };
  const decideCodingCandidate=async(build:CandidateBuild,decision:"approved"|"rejected")=>{
    if(!build.coding_run_id||!build.coding_approval_action_hash)throw new Error("The frozen coding approval is unavailable.");
    const verb=decision==="approved"?"execute this exact sandbox plan and publish its draft PR":"reject this frozen plan";
    if(!window.confirm(`Human approval required: ${verb}. Continue?`))return;
    const response=await fetch(`${API}/coding/runs/${build.coding_run_id}/decision`,{
      method:"POST",headers:{...headers(),"Content-Type":"application/json"},
      body:JSON.stringify({action_hash:build.coding_approval_action_hash,decision,note:decision==="approved"?"Approved from improvement review":"Rejected from improvement review"}),
    });
    const data=await response.json();
    if(!response.ok)throw new Error(data.detail??"Coding-plan decision failed");
    setMessage(decision==="approved"?"Exact frozen plan approved. A fresh credential-free sandbox will execute and validate it before creating a draft PR.":"Frozen coding plan rejected; no patch was published.");
    await load();
  };
  ...
  const requiredActions=proposals.filter(proposal=>
    (proposal.status==="awaiting_review"&&proposal.candidate_state==="implementation_draft")||
    (proposal.status==="awaiting_review"&&proposal.candidate_state==="validated_implementation")||
    (proposal.status==="approved_for_canary"&&proposal.deployment_evidence?.verified===true)||
    proposal.status==="awaiting_promotion"
  );
  const retryActions=candidateBuilds.filter(build=>build.new_policy_retry_available);
  const codingApprovalActions=candidateBuilds.filter(build=>build.coding_run_status==="awaiting_approval"&&build.coding_approval_action_hash);
  const evidenceActions=proposals.filter(proposal=>proposal.candidate_manifest?.build_admission==="evidence_required");
  ...
  return <main className="mx-auto min-h-screen max-w-5xl bg-zinc-50 p-6 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
    ...
    <section className="mb-8 space-y-4"><div><h2 className="text-xl font-semibold">Failure review inbox</h2><p className="text-sm text-zinc-500">Newest failures appear first. Expand one occurrence to compare its two bounded strategies. Human candidate, canary, promotion, and publication gates remain mandatory.</p></div>{incidents.length===0&&<p className="rounded-xl border bg-white p-5 dark:bg-zinc-900">No failures are awaiting review.</p>}{incidents.map((incident,index)=><details key={incident.id} open={index===0} className="rounded-xl border bg-white p-5 dark:bg-zinc-900"><summary className="cursor-pointer list-none"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex flex-wrap gap-2 text-xs uppercase"><span className="rounded bg-red-100 px-2 py-1 text-red-800">{incident.risk_level}</span><span>{incident.stage} · {incident.category}</span><span>{incident.cluster_occurrences} in cluster</span>{index===0&&<span className="rounded bg-blue-100 px-2 py-1 text-blue-800">Newest</span>}</div><h3 className="mt-2 text-lg font-semibold">{incident.title}</h3></div><div className="text-right text-xs text-zinc-500"><div>{timestamp(incident.created_at)}</div><div>Expand/collapse</div></div></div><p className="mt-2 text-sm">{incident.summary}</p></summary><div className="mt-4 border-t pt-4"><p className="text-sm text-zinc-500">Incident {incident.id} · Intent: {incident.intent_kind.replaceAll("_"," ")} · Component: {incident.component} · Breaking point: {incident.breaking_point??"not recorded"}</p><p className="mt-2 rounded bg-zinc-100 p-3 text-sm dark:bg-zinc-800">Sanitized request: {incident.request_excerpt}</p><p className="mt-3 text-sm"><strong>Root cause:</strong> {incident.root_cause}</p><div className="mt-4 grid gap-3 md:grid-cols-2">{incident.improvement_options.map(option=><div key={option.id} className={`rounded-xl border p-4 ${incident.recommended_option===option.id?"border-blue-500":""}`}><h4 className="font-semibold">Option {option.id}: {option.title}</h4>{incident.recommended_option===option.id&&<p className="text-xs text-blue-600">Recommended — {incident.recommendation_reason}</p>}<p className="mt-2 text-sm">{option.explanation}</p><p className="mt-2 text-xs text-zinc-500">Tradeoff: {option.tradeoff}</p><button onClick={()=>safely(()=>decideIncident(incident,`choose_${option.id}` as "choose_A"|"choose_B"))} className="mt-3 rounded-lg bg-blue-600 px-3 py-2 text-sm text-white">Choose option {option.id}</button></div>)}</div><div className="mt-3 flex gap-2"><button onClick={()=>safely(()=>decideIncident(incident,"acknowledged"))} className="rounded border px-3 py-2 text-sm">Acknowledge</button><button onClick={()=>safely(()=>decideIncident(incident,"ignored"))} className="rounded border px-3 py-2 text-sm">Ignore</button></div></div></details>)}</section>
    ...
    <section className="mb-8 rounded-xl border bg-white p-5 dark:bg-zinc-900"><h2 className="text-xl font-semibold">Candidate builder</h2><p className="text-sm text-zinc-500">Newest builds appear first. Expand a build for its complete model chain, checkpoint, budget, failure, and retry evidence. Groq-generated files only run later in trusted no-secret CI.</p><div className="mt-4 space-y-3">{candidateBuilds.length===0&&<p className="text-sm text-zinc-500">No candidate builds have been recorded.</p>}{candidateBuilds.map((build,index)=><details key={build.id} open={index===0} className="rounded-xl border p-4"><summary className="cursor-pointer list-none"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2 text-xs uppercase"><span className="rounded bg-zinc-100 px-2 py-1 dark:bg-zinc-800">{build.status.replaceAll("_"," ")}</span><span>{build.mode.replaceAll("_"," ")}</span>{build.build_admission==="evidence_required"&&<span className="rounded bg-amber-100 px-2 py-1 text-amber-800">evidence required</span>}{index===0&&<span className="rounded bg-blue-100 px-2 py-1 text-blue-800">Newest</span>}</div><h3 className="mt-2 break-all font-semibold">{build.proposal_key}</h3><p className="mt-1 text-xs text-zinc-500">Build {build.id}</p></div><div className="text-right text-xs text-zinc-500"><div>Created {timestamp(build.created_at)}</div><div>Updated {timestamp(build.updated_at)}</div><div>Expand/collapse</div></div></div><p className="mt-2 text-sm">{build.generation_phase?.replaceAll("_"," ")??"not checkpointed"}{build.active_role?` · ${build.active_role.replaceAll("_"," ")}`:""}{build.next_round!==undefined&&build.next_round!==null?` · next round ${build.next_round}`:""}</p></summary><div className="mt-4 grid gap-4 border-t pt-4 text-xs md:grid-cols-2"><div><h4 className="font-semibold">Model and policy</h4><p className="mt-1">{(build.models_used?.length?build.models_used:[build.model_name]).join(" → ")}</p><p className="text-zinc-500">Configured primary: {build.model_name}<br/>{build.model_policy_version}<br/>{build.tool_policy_version}</p></div><div><h4 className="font-semibold">Dispatch and checkpoint</h4><p className="mt-1">Dispatch: {build.retry_dispatch_state?.replaceAll("_"," ")??"not dispatched"}</p><p className="text-zinc-500">Gate: {build.progress_gate?.replaceAll("_"," ")??"not reached"}{build.last_tool_name?` · last ${build.last_tool_name}`:""}</p></div><div><h4 className="font-semibold">Budget and repository progress</h4><p className="mt-1">Base {build.base_token_budget.toLocaleString()} · Effective {build.effective_token_budget.toLocaleString()}</p><p>Usage: cumulative {build.cumulative_tokens.toLocaleString()} · current role {build.active_role_tokens.toLocaleString()}</p><p className="text-zinc-500">{build.staged_file_count} staged/frozen · {build.tool_calls} tools · {build.read_bytes.toLocaleString()} read bytes · {build.json_tool_protocol?"JSON protocol":"native protocol"}</p></div><div><h4 className="font-semibold">Resume</h4><p className="mt-1">{build.retryable?"Provider retry eligible":"Provider retry unavailable"}: {build.retry_reason.replaceAll("_"," ")}</p><p className="text-zinc-500">{build.resume_point?.phase?`Resume ${build.resume_point.phase.replaceAll("_"," ")}`:"No safe resume point"}{build.resume_point?.active_role?` · ${build.resume_point.active_role.replaceAll("_"," ")}`:""}{build.resume_point?.next_round!==undefined?` round ${build.resume_point.next_round}`:""}{build.resume_point?.staged_file_count!==undefined?` · ${build.resume_point.staged_file_count} frozen files`:""}</p>{build.retryable&&<p className="text-zinc-500">{build.next_retry_at?`Eligible ${timestamp(build.next_retry_at)}`:"Waiting for scheduler"}</p>}</div><div className="md:col-span-2"><h4 className="font-semibold">Failure and next action</h4><p className="mt-1">{build.error_message??"No terminal failure recorded."}</p>{build.retry_error_type&&<p className="text-zinc-500">{build.retry_stage??"runner"} · {build.retry_error_type}</p>}{build.contract_errors.length>0&&<p className="text-zinc-500">Contract: {build.contract_errors.join(", ")}</p>}{build.admission_reason_codes.length>0&&<p className="text-amber-700">Admission: {build.admission_reason_codes.join(", ").replaceAll("_"," ")}. Collect specific evidence instead of retrying this diagnosis.</p>}{build.new_policy_retry_available&&<button onClick={()=>safely(()=>retryWithCurrentPolicy(build))} className="mt-2 rounded border px-2 py-1">Create current-policy attempt</button>}</div></div></details>)}</div></section>
    <section className={`mb-8 rounded-xl border-2 p-5 ${requiredActions.length||retryActions.length||codingApprovalActions.length||evidenceActions.length?"border-amber-500 bg-amber-50 text-amber-950":"border-emerald-300 bg-emerald-50 text-emerald-950"}`}>
      <h2 className="text-xl font-semibold">Required human actions</h2>
      <p className="mt-1 text-sm">{requiredActions.length||retryActions.length||codingApprovalActions.length||evidenceActions.length?"These governed actions are waiting. Nothing below is silently approved, published, activated, or promoted.":"No candidate-builder approval, evidence, or retry action is currently waiting."}</p>
      <div className="mt-4 space-y-3">
        {codingApprovalActions.map(build=><div key={`coding-approval-${build.id}`} className="rounded border-2 border-amber-500 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><strong>Exact coding plan awaiting approval: {build.title}</strong><p className="mt-1 text-sm">Run {build.coding_run_id} · expires {timestamp(build.coding_approval_expires_at)}</p><p className="text-sm">Files: {(build.coding_approval_preview?.changes??[]).map(change=>change.path).filter(Boolean).join(", ")||"manifest available after refresh"}</p></div><div className="flex gap-2"><button onClick={()=>safely(()=>decideCodingCandidate(build,"approved"))} className="rounded bg-blue-700 px-3 py-2 text-sm text-white">Approve exact sandbox plan</button><button onClick={()=>safely(()=>decideCodingCandidate(build,"rejected"))} className="rounded border border-red-600 px-3 py-2 text-sm text-red-700">Reject plan</button></div></div></div>)}
        {retryActions.map(build=><div key={`retry-${build.id}`} className="flex flex-wrap items-center justify-between gap-3 rounded border border-amber-300 bg-white p-3"><div><strong>{build.proposal_key}</strong><p className="text-sm">The prior attempt is terminal and a newer bounded builder policy is available.</p></div><button onClick={()=>safely(()=>retryWithCurrentPolicy(build))} className="rounded bg-amber-700 px-3 py-2 text-sm text-white">Create current-policy attempt</button></div>)}
        {evidenceActions.map(proposal=><div key={`evidence-${proposal.proposal_key}`} className="rounded border border-amber-300 bg-white p-3"><strong>{proposal.title}</strong><p className="text-sm">Diagnosis only: specific reproducible failure evidence must be captured before candidate generation. No Groq tokens, CI, deployment, or approval are being requested yet.</p></div>)}
        {requiredActions.map(proposal=><div key={`action-${proposal.proposal_key}`} className="flex flex-wrap items-center justify-between gap-3 rounded border border-amber-300 bg-white p-3"><div><strong>{proposal.title}</strong><p className="text-sm">{proposal.candidate_state==="implementation_draft"?"Frozen files are ready for a draft PR and trusted CI.":proposal.status==="approved_for_canary"?"The isolated deployment is verified and awaits traffic activation.":proposal.status==="awaiting_promotion"?"Canary evidence is ready for a promotion decision.":"Trusted CI passed; approve an isolated canary deployment or request changes."}</p></div><div className="flex flex-wrap gap-2">{proposal.candidate_state==="implementation_draft"&&<button onClick={()=>safely(()=>publish(proposal,"github"))} className="rounded bg-zinc-950 px-3 py-2 text-sm text-white">Publish frozen draft PR</button>}{proposal.status==="awaiting_review"&&proposal.candidate_state==="validated_implementation"&&<button onClick={()=>safely(()=>decide(proposal,"approved"))} className="rounded bg-blue-600 px-3 py-2 text-sm text-white">Approve isolated canary</button>}{proposal.status==="approved_for_canary"&&<button onClick={()=>safely(()=>decide(proposal,"approved","activate-canary"))} className="rounded bg-blue-600 px-3 py-2 text-sm text-white">Activate canary traffic</button>}{proposal.status==="awaiting_promotion"&&<button onClick={()=>safely(()=>decide(proposal,"approved","promotion-decision"))} className="rounded bg-emerald-600 px-3 py-2 text-sm text-white">Promote to production</button>}</div></div>)}
      </div>
    </section>
